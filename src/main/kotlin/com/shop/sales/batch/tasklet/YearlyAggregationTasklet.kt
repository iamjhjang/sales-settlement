...
class YearlyAggregationTasklet(
    ...
    override fun execute(contribution: StepContribution, chunkContext: ChunkContext): RepeatStatus {
        ...
        jdbc.update(
            """
            insert into tb_sales_yearly(year_key, brand_id, item_code, order_cnt, qty_sum, amount_sum, updated_at)
            select
                to_char(s.business_date, 'YYYY') as year_key,
                s.brand_id,
                s.item_code,
                count(*) as order_cnt,
            ...
            where s.business_date >= :yearStart and s.business_date < :nextYearStart
            group by to_char(s.business_date, 'YYYY'), s.brand_id, s.item_code
            on conflict (year_key, brand_id, item_code)
            do update set
